...
lazy_static!{
     static ref DEFAULT_PASSPORT: RwLock<PassportEntity> = {
        let start = Utc.datetime_from_str("2023-02-07 00:00:00", "%Y-%m-%d %H:%M:%S").expect("default passport entity,start time generate error");
        let end = Utc.datetime_from_str("2123-02-07 00:00:00", "%Y-%m-%d %H:%M:%S").expect("default passport entity,end time generate error");
        let interval = Duration::from_secs(60*60*24*30*12*10);
        let entity = PassportEntity::new("wd_passport_default_key", (start, end), interval, 1024).expect("rsa certs build failed");
        RwLock::new(entity)
    };
}

pub fn init_passport(key:&[u8],bit_size:usize,start:&str,end:&str,interval:u64)->anyhow::Result<()>{
...
pub fn format_public_pem()->Vec<(i64,String)>{
    let rpp = DEFAULT_PASSPORT.read().expect("format_public_pem DEFAULT_PASSPORT error");
    rpp.to_public_pem()
}